...
\begin{document}
	...
	\begin{itemize}
			...
			Hier wird das Brett mit dem Spiel an sich angezeigt. Hier kann man zudem die einzelnen Kacheln anwählen und somit Züge ausführen. Außer dem Spielfeld gibt es die Buttons  \glqq Unentschieden\grqq\ und \glqq Aufgeben\grqq .
					 \begin{description}
									 
					 \item \textbf{- drawClicked(): void}\\ Wird ausgeführt, wenn der Button \glqq Unentschieden\grqq\ gedürckt wird. Schickt dem Gegner einen \textbf{DrawDialog}, mit welchem er das Remis-Angebot annehmen oder ablehnen kann. 
					 \item \textbf{- giveUpClicked(): void}\\ Wird ausgeführt, wenn der Button \glqq Aufgeben\grqq\ gedrückt wird. Dem Spieler erscheint ein \textbf{GiveUpDialog}, mit welchem er sein Aufgeben bestätigen oder abbrechen kann. 
					  \item \textbf{- tileSelected(): void}\\ Wird ausgeführt, wenn der Benutzer eine Kachel auf dem Spielfeld anklickt. Erzeugt aus den Koordinaten, auf welche der Benutzer geklickt hat, eine \textbf{Position}. Prüft mittel der \textbf{whiteToMove()}-Methode des Game-Objekts, ob der Benutzer am Zug ist. Wenn ja, wird zuerst geprüft, ob der Benutzer bereits eine Kachel gewählt hatte. Ist dies der Fall, wird \textbf{executeMove()} ausgeführt, hat der Benutzer noch keine Kacheln gewählt wird \textbf{showPossiblePositions()} ausgeführt. 		  							\item \textbf{- showPositions(start: Position): void}\\ Prüft zunächst mithilfe von \textbf{getPieceAt(position: Position} des Game-Objektes, ob sich an der gegebenen Position eine eigene Figur befindet. Ist dies der Fall, wird \textbf{getPossibleMoves()} aufgerufen und die Zielpositionen der jeweiligen Züge werden auf dem Brett angezeigt, ansonsten passiert nichts.
...
\end{description}			 
		...
		\begin{itemize}		
			...
			Die AppComaptActivity wird als Dialog verwendet.\\
				...
				\end{description}
					...
					\begin{description}					
					\item \textbf{- accepted(): void}\\ Sendet dem Server die Information, dass das Spiel mit einem Unentschieden beedndet wurde. Dieser zeigt beiden Spielern einen entsprechenden \textbf{ResultDialog}.
		...
		\begin{itemize}
			...
			\item \textbf{\Large{MoveHandler}}
				...
				\rule{\textwidth}{1pt}	
				\begin{description}
					\item \textbf{+ processMove(move: String): void}\\ Laden und Speichern erfolgen wieder über einen \textbf{DatabaseHandler}. Überprüft, ob die Kombination aus Brett und Zug gültig ist. Wendet den Zug an, falls dieser gütlig ist. Speichert das neue Brett ab und benachrichtigt anschließend den Gegner.\\
					\\
				\end{description}
