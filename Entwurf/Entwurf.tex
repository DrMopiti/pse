...
\begin{document}
	...
	\section{Entwurfsentscheidungen}
		...
		\subsection{GUI}
		Die GUI wurde mit den Klassen android.app.Activity und android.support.v7.app.AppCompatActivity aus der Oberklasse android.content.Context erstellt. Alle Aktivitäten erben von der Activity-Klasse und die Dialoge von AppComaptActivity-Klasse. Durch die Aktivity kann der Benutzer eine bestimmte Aufgabe ausführen. In der Klasse AppCompatActivity wird außerdem der AlertDialog verwendet. Dadurch kann man die Knöpfe anzeigen lassen, die somit eine Interaktion der Benutzeroberfläche erzeugen.
	...
	\section{Klassendiagramme}
			  ...
			  \end{itemize}	
				...
				\end{description} 
	 			...
	 			\begin{description}
					...
					\item \textbf{+ whiteToMove(): boolean}\\ Führt \textbf{whiteToMove} auf \textbf{board} aus, gibt zurück, ob Weiß am Zug ist.
		...
		\begin{itemize}
			...
			Konkreter Regellieferer, welcher die genauen Schachregeln zur Verfügung stellt.\\
				...
				\begin{description}
					...
					\item \textbf{+ getLegalMoves(position: Position, board: BoardState): List<Move>}\\ Gibt eine Liste an erlaubten Zügen ausgehend von einer ausgewählten Position und einem Brett zurück. Dazu werden zunächst mit \textbf{getPossibleMoves(Position, BoardState)} alle möglichen Züge berechnet. Anschließend wird jeder Zug auf einer Kopie des Brettes simuliert, und mithilfe von \textbf{isChecked(boolean, BoardState)} überprüft, ob der ziehende Spieler nach seinem Zug im Schach stünde (was den Zug ungültig machen würde).
					...
					\item \textbf{- isDraw(board: BoardState): boolean}\\ Prüft, ob ein anderweitiges Unentschieden erreicht wurde. Dies ist der Fall, wenn zu wenig Figuren auf dem Brett vorhanden sind, um Matt zu setzen oder wenn 50 Züge lang keine Figur geschlagen und kein Bauer gezogen wurde, also wenn der \textbf{movesWithoutAction}-Wert von \textbf{board} 50 (oder größer) ist.
				\end{description}
		\end{itemize}
		\newpage
		...
		\begin{itemize}
			...
			\\
				...
				\begin{description}
					...
					\item \textbf{+ BoardState(representation: String)} Erzeugt ein Brett, ausgehend von einem String. In diesem müssen alle notwendigen Informationen in einem einheitlichen Format gespeichert sein.
					\item \textbf{+ applyMove(move: Move): void}\\ Führt einen Zug auf dem Brett aus, indem es die Figur(en) wie im Move-Objekt vorgegeben bewegt und möglicherweise andere Figuren schlägt(überschreibt). Außerdem wird die zu ziehende Farbe, der letzte Zug, sowie die Anzahl der Züge ohne Aktion aktualisiert. Bei entsprechender Verletzung werden mögliche Rochaden auf \textbf{false} gesetzt.
					...
					\newpage
					\item \textbf{+ whiteToMove(): boolean}\\ Gibt \textbf{whiteToMove} zurück.
		...
		\begin{itemize}
			...
			Stellt einen Spieler eines konkreten Spieles dar. Existiert nur während das jeweilige Spiel existiert.\\
			\\
				\textbf{\large{Attribute}}
				\newline
				\rule{\textwidth}{1pt}
		...
		\begin{itemize}
			...
			\item \textbf{\Large{King}}
				...
				\begin{description}
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState): List<Move>}\\ Gibt eine Liste der Züge zurück, die ein König auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann, inklusive Rochade. Dabei wird nicht berücksichtigt, ob sich der König nach dem Zug im Schach befinden würde.\\
					\\
				\end{description}
			...
			\item \textbf{\Large{Queen}}
				...
				\begin{description}	
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState): List<Move>}\\ Gibt eine Liste der Züge zurück, die eine Dame auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug im Schach befinden würde.\\
					\\
				\end{description}
			...
			\item \textbf{\Large{Rook}}
				...
				\begin{description}	
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState)}\\ Gibt eine Liste der Züge zurück, die ein Turm auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug im Schach befinden würde.\\
					\\
				\end{description}
			...
			\item \textbf{\Large{Bishop}}
				...
				\begin{description}				
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState)}\\ Gibt eine Liste der Züge zurück, die ein Läufer auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug im Schach befinden würde.\\
					\\
				\end{description}
			...
			\item \textbf{\Large{Knight}}
				...
				\begin{description}
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState)}\\ Gibt eine Liste der Züge zurück, die ein Springer auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug im Schach befinden würde.\\
					\\
				\end{description}
			...
			\item \textbf{\Large{Pawn}}
				...
				\begin{description}
					...
					\item \textbf{+ getMovement(p: Position, b: BoardState)}\\ Gibt eine Liste der Züge zurück, die ein Bauer auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann, inklusive en passant, Doppelsprung und Umwandlung. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug im Schach befinden würde.
				\end{description}
		\end{itemize}		
		\newpage		
	...
	\begin{itemize}
			...
			Ist eine einzelne fokussierte Aufgabe, die der Benutzer ausführen kann. Da fast alle Aktivitäten mit dem Benutzer interagieren, kümmert sich die Activity-Klasse darum, ein Fenster für sie zu erstellen.\\
				...
				\begin{description}			
					\item \textbf{\# onCreate(savedInstanceState: Bundle)}\\ Wird aufgerufen, wenn die Aktivität erstellt wird. Wenn die Aktivität neu initialisiert wird, enthält das Bundle die Daten, die es zuletzt bei SaveInstanceState bereitgestellt hat. Ansonsten ist es \textbf{null}.
					\item \textbf{\# onStart(): void}\\ Wird aufgerufen, wenn die Aktivität gestoppt wurde, jetzt aber dem Benutzer wieder angezeigt wird.
					\item \textbf{\# onStop(): void}\\ Wird aufgerufen, wenn die Aktivität für den Benutzer nicht mehr sichtbar sind.
					\item \textbf{\# onResume(): void}\\ Wird aufgerufen, damit die Aktivität mit dem Benutzer interagieren kann. 
					\item \textbf{+ onCreateOptionsMenu(menu: Menu): boolean}\\ Initialisiert den Inhalt des Standardoptionen-Menüs der Aktivität. Es gibt das Optionsmenü, in dem die Elemente platziert werden sollen. Es gibt true für das angezeigte Menü zurück, bei false wird es nicht angezeigt.
			...
			Hier wird das Brett mit dem Spiel an sich angezeigt. Man kann zudem die einzelnen Kacheln anwählen und somit Züge ausführen. Außer dem Spielfeld gibt es die Buttons  \glqq Unentschieden\grqq\ und \glqq Aufgeben\grqq .
					 \begin{description}
									 
					 \item \textbf{- drawClicked(): void}\\ Wird ausgeführt, wenn der Button \glqq Unentschieden\grqq\ gedrückt wird. Schickt dem Gegner einen \textbf{DrawDialog}, mit welchem er das Remis-Angebot annehmen oder ablehnen kann. 
					 \item \textbf{- giveUpClicked(): void}\\ Wird ausgeführt, wenn der Button \glqq Aufgeben\grqq\ gedrückt wird. Dem Spieler erscheint ein \textbf{GiveUpDialog}, mit welchem er sein Aufgeben bestätigen oder abbrechen kann. 

					  \item \textbf{- tileSelected(): void}\\ Wird ausgeführt, wenn der Benutzer eine Kachel auf dem Spielfeld anklickt. Erzeugt aus den Koordinaten, auf welche der Benutzer geklickt hat, eine \textbf{Position}. Prüft mittel der \textbf{whiteToMove()}-Methode des Game-Objekts, ob der Benutzer am Zug ist. Wenn ja, wird zuerst geprüft, ob der Benutzer bereits eine Kachel gewählt hatte. Ist dies der Fall, wird \textbf{executeMove()} ausgeführt, hat der Benutzer noch keine Kacheln gewählt wird \textbf{showPossiblePositions()} ausgeführt. 		  						
					  \item \textbf{- showPositions(start: Position): void}\\ Prüft zunächst mithilfe von \textbf{getPieceAt(position: Position} des Game-Objektes, ob sich an der gegebenen Position eine eigene Figur befindet. Ist dies der Fall, wird \textbf{getPossibleMoves()} aufgerufen und die Zielpositionen der jeweiligen Züge werden auf dem Brett angezeigt, ansonsten passiert nichts.
					  \item \textbf{- executeMove(goal: Position): void}\\ Prüft zunächst, ob die ausgewählte \textbf{Position} auf dem Brett markiert war. Wenn ja, wird der entsprechende Zug mit \textbf{applyMove()} ausgeführt. Wenn nicht, werden die Markierungen wieder entfernt, und der Benutzer muss erneut eine seiner Figuren anwählen, um die möglichen Züge angezeigt zu bekommen.
					  
					 \end{description}
			
			\item \textbf{\Large{MainMenuActivity}}
			
			Hier wird das Hauptmenü angezeigt mit den Buttons \glqq Sofortspiel\grqq , \glqq Spielersuche\grqq\ und \glqq Rangliste/Statistik\grqq .
					\begin{description}
					
					\item \textbf{- quickMatchClicked(): void}\\ Sobald der Benutzer \textbf {„Sofortspiel“} drückt, öffnet sich die \textbf {„BoardActivity“}. Dort spielt man gegen einen zufälligen Gegner.
				
					\item \textbf{- searchPlayerClicked(): void}\\ Sobald der Benutzer \textbf {„Spielersuche“} drückt, öffnet sich die \textbf {„SearchPlayerActivity“}. Dort kann man einen anderen Benutzer mit seinem Spielernamen suchen und mit dem Button  \textbf {„Herausfordern“} diesen herausfordern. 
					
					\item \textbf{- statisticsClicked(): void}\\ Sobald der Benutzer \textbf {„Rangliste/Statistik“} drückt, öffnet sich eine \textbf {„StatisticActivity“}, welche den Namen des Benutzers als Parameter erhält. Dort kann der Benutzer seine Statistik einsehen.  \\
					
					\end{description}
			
			\item \textbf{\Large{StatisticsActivity}}
			
			Zeigt die Statistiken eines belibigen Spielers an. Ist die Statistik nicht von dem Benutzer, welcher sie aufruft, gibt es unten einen Button \textbf{Herausfordern}.
			\begin{description}
			\item \textbf{- challengeClicked(): void}\\
			Sendet dem Gegner einen \textbf{ChallengedDialog} mit welchem er die Herausforderung annehmen oder ablehnen kann. \\
\end{description}			 
...
\newpage
				 ...
				 \item \textbf{\Large{SearchPlayerActivity }} 
					...
					Nach dem drücken des Button \textbf {SearchPlayer} in der \textbf{MainMenuActivity} öffnet sich die \textbf {SearchPlayerActivity}. Dort kann der Benutzer mithilfe einer Suchfunktion andere Gegner finden. Klickt er auf einen Spielernamen, öffnet sich eine \textbf{StatisticsActivity} mit den Statistiken des jeweiligen Spielers und mit dem Button \textbf {„Herausfordern“}.
					\begin{description}
					\item \textbf{-playerSelected(name: String): void} \\ Ruft eine \textbf{StatisticsActivity} mit den Statistiken des entsprechenden Spielers auf. \\	
					\end{description}
		...
		\begin{itemize}		
			...
			Die AppCompatActivity wird als Dialog verwendet.\\
					...
					\item\textbf{+ onClick(view: View)}:\\ Erzeugt die Funktionalität eines Knopfes, wobei ein AlertDialog geöffnet wird.
					\\
				...
				\end{description}
					...
					Fragt einen Spieler, ob er das Remis-Angebot des Gegners annehmen oder ablehnen will, dafür stehen ihm die zwei Buttons \glqq Annehmen\grqq\ und \glqq Ablehnen\grqq\ zur Verfügung.\\
					\begin{description}					
					\item \textbf{- accepted(): void}\\ Sendet dem Server die Information, dass das Spiel mit einem Unentschieden beendet wurde. Dieser zeigt beiden Spielern einen entsprechenden \textbf{ResultDialog}.
					\item \textbf{- declined(): void}\\ Schließt den Dialog und sendet dem anderen Benutzer einen \textbf{DeniedDialog}.			
					\end{description}
					...
					\begin{description}					
					\item \textbf{- confirmed(): void}\\ Sendet dem Server die Information, dass das Spiel beendet wurde. Dieser zeigt beiden Spielern einen entsprechenden \textbf{ResultDialog}.
					\item \textbf{- aborted(): void}\\ Schließt den Dialog.					
					\end{description}
					
					
					\item \textbf{\Large{ResultDialog}} 
					...
					\begin{description}					
					\item \textbf{- backToMenu(): void}\\ Schließt den Dialog und öffnet die \textbf{MainMenuActivity}.
					\item \textbf{- saveGame(): void}\\ Speichert das Spiel als String ab.
					\item \textbf{- rematch(): void}\\ Sendet dem Gegner einen erneuten \textbf{ChallengeDialog}. Dieser kann dann erneut ein Spiel akzeptieren oder ablehnen.			
					\end{description}
					
					\item \textbf{\Large{PawnTransformationDialog }} 
					
					Ein Dialog zur Auswahl einer Figur, in welche sich ein Bauer verwandeln soll, wenn dieser die gegnerische Grundreihe erreicht hat. Dazu hat der Benutzer die Buttons \glqq Queen\grqq , \glqq Rook\grqq , \glqq Bishop\grqq\ und \glqq Knight\grqq\ zur Verfügung.
					
					
					\begin{description}					
					\item \textbf{- queenSelected(): void}\\ Erzeugt einen \textbf{Promotion}-Move mit einer \textbf{Queen} als Parameter und wendet diesen auf das Brett an.
					\item \textbf{- rookSelected(): void}\\ Erzeugt einen \textbf{Promotion}-Move mit einem \textbf{Rook} als Parameter, und wendet diesen auf das Brett an.
					\item \textbf{- bishopSelected(): void}\\ Erzeugt einen \textbf{Promotion}-Move mit einem \textbf{Bishop} als Parameter und wendet diesen auf das Brett an.
					\item \textbf{- knightSelected(): void}\\Erzeugt einen \textbf{Promotion}-Move mit einem \textbf{Knight} als Parameter und wendet diesen auf das Brett an.				
					\end{description}	
					...
					\begin{description}					
					\item \textbf{- accepted(): void}\\ Erzeugt ein neues Spiel zwischen den beiden Spielern und informiert den Server darüber.
					\item \textbf{- declined(): void}\\	Schließt den Dialog; der andere Spieler erhält einen	\textbf{DeniedDialog}.			
					\end{description}
					...
					\begin{description}
					\item \textbf{- okClicked(): void}\\ Schließt den Dialog.
					\end{description}
		...
		\begin{itemize}
			...
			\item \textbf{\Large{MoveHandler}}
				...
				\rule{\textwidth}{1pt}	
				\begin{description}
					\item \textbf{+ processMove(move: String): void}\\ Laden und Speichern erfolgen wieder über einen \textbf{DatabaseHandler}. Überprüft, ob die Kombination aus Brett und Zug gültig ist. Wendet den Zug an, falls dieser valide ist. Speichert das neue Brett ab und benachrichtigt anschließend den Gegner.\\
					\\
				\end{description}
