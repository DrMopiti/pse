	...
	\section{Klassendiagramme}
		...
		\begin{itemize}
			...
			\item \textbf{\textit{getLegalMoves(Position position, BoardState board):}} Gibt eine Liste an erlaubten Zügen ausgehend von einer ausgewählten Position und einem Brett zurück. Dazu werden zunächst mit \textbf{\textit{getPossibleMoves(Position, BoardState)}} alle möglichen Züge berechnet. Anschließen wird jeder Zug auf einer Kopie des Brettes simuliert, und mithilfe von \textbf{\textit{isChecked(boolean, BoardState)}} überprüft, ob der selbe Spieler danach im Schach stünde (was den Zug ungültig machen würde).
				...
				\item \textbf{\textit{getPossibleMoves(Position position, BoardState board):}} Gibt alle möglichen Züge einer Figur auf dem Brett zurück, ohne dabei zu berücksichtigen, ob der ziehende Spieler nach diesem Zug im Schach stehen würde. Dazu wird die Figur an der entsprechenden Stelle bestimmt und anschließend je eine(bzw. zwei) der folgenden 5 Methoden aufgerufen:
				\item \textbf{\textit{getDiagonalMoves(Position position, BoardState board):}} Gibt alle Züge zurück, welche eine Figur in alle 4 diagonalen Richtungen ausführen kann. Wird benutzt für Läufer und Dame.
				\item \textbf{\textit{getHorizontalAndVerticalMoves(Position position, BoardState board):}} Gibt alle Züge zurück, welche eine Figur in horizontaler oder vertikaler Richtung ausführen kann. Wird benutzt für Turm und Dame.
				\item \textbf{\textit{getKingMoves(Position position, BoardState board):}} Gibt die möglichen Züge eines Königs einschließlich möglicher Rochaden zurück.
				\item \textbf{\textit{getKnightMoves(Position position, BoardState board):}} Gibt die möglichen Züge eines Springers zurück.
				\item \textbf{\textit{getPawnMoves(Position position, BoardState board):}} Gibt die möglichen Züge eines Bauern einschließlich möglichem Doppelschritt, enPassant sowie Umwandlung.
...
\begin{minipage}{\linewidth}
		...
		\begin{itemize}
			\item \textbf{\textit{Piece}}
				\begin{description}
				\item Stellt das abstrakte Konzept einer Schachfigur dar.
			...
			\item \textbf{\textit{King}}
				...
				\begin{description}
				\item Stellt einen Springer dar.
