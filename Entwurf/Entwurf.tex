	...
	\section{Klassendiagramme}
		...
		\begin{itemize}
			\item \textbf{\textit{Spiel}}
				\begin{description}
					\item \textbf{\textit{Game}} stellt ein Spiel dar. Es besteht aus zwei Spielern \textbf{\textit{Player}} und einem Spielfeld \textbf{\textit{BoardState}}. Außerdem benutzt es eine Instanziierung des Interface \textbf{\textit{RuleProvider}}, welches die Regeln für ein beliebiges Spiel auf einem Schachbrett liefert.
					\item In diesem Fall wird das Interface nur von der Klasse \textbf{\textit{ChessRuleProvider}} implementiert, welche die Schachregeln liefert. Diese Klasse benutzt als Rückgabewert einer Methode die Klasse \textbf{\textit{Result}}.
					\item Ein \textbf{\textit{Player}} ist ein Spieler eines konkretes Spiels und besteht aus einem \textbf{\textit{User}}, welcher einen Benutzer im Allgemeinen darstellt. Umgekehrt muss ein \textbf{\textit{User}} aber nicht zu jedem Zeitpunkt einen \textbf{\textit{Player}} haben.
					\item Das Spielfeld \textbf{\textit{BoardState}} enthält 64  \textbf{\textit{Tiles}} (Schachfelder) und einen \textbf{\textit{Move}}, welcher hier den letzten ausgeführten Zug des Spiels darstellt. 
					...
					\item Ein \textbf{\textit{Move}} besteht im Allgemeinen nur aus zwei \textbf{\textit{Positionen}}. Es gibt drei Spezialfälle eines Zuges, welche eine gesonderte Implementierung benötigen, da neben zwei \textbf{\textit{Positionen}} noch andere Attribute enthalten.
					\item \textbf{\textit{Position}} stellt eine Position auf einem Schachbrett dar und besteht aus zwei Zahlen, welche die Koordinaten darstellen. Diese müssen zwischen 0-7 liegen.
				\end{description}
		...
		\end{minipage}
			...
			\item letzer ausgeführter Spielzug
			\item Anzahl der Züge ohne Bauernzug oder Schlagen von Figuren
			  \end{itemize}	
				...
				\end{description} 
		 		...
		 		\rule{\textwidth}{1pt}
	 			\begin{description}
					 \item \textbf{+ Game(User user1, User user2)}\\ Konstruktor, welcher ein Spiel mit den Standard Schachregeln und einem Brett auf Anfangsposition erzeugt. Der zuerst übergebene User erhält die weißen Figuren, der zweite die schwarzen.
					 \item \textbf{+ Game(User user1, User user2, BoardState board)}\\ Konstruktor, welcher ein Spiel mit den Standard Schachregeln und dem übergebenen Brettstatus erzeugt. Der zuerst übergebene User erhält die weißen Figuren, der zweite die schwarzen.
					 \item \textbf{+ getBoard(): BoardState}\\ Gibt den Brettstatus \textbf{board}des akutellen Spiels zurück.
					\item \textbf{+ getBoard(BoardState board): void}\\ Setzt den Brettstatus \textbf{board} auf das übergebene Objekt.
					...
					\item \textbf{+ applyMove(Move): void}\\ Führt den übergebenen Zug auf dem Spielbrett \textbf{board} aus.
					\item \textbf{+ applyMove(String): void}\\ Führt den als String übergebenen Zug auf dem Spielbrett \textbf{board} aus. Dazu muss der String erst in ein Objekt vom Typ \textbf{Move} umgewandelt werden.
		...
		\begin{itemize}
			...
			\\			
				...
				\begin{description}			
					...
					\item \textbf{+ getLegalMoves(Position position, BoardState board): List<Move>}\\ Soll auf einem Brett ausgehend von einer Position die Zugmöglichkeiten der sich darauf befindenden Figur berechnen, entsprechend der implementierten Regeln.
					\item \textbf{+ isLegalMove(Move move, BoardState board): boolean}\\ Soll überprüfen, ob ein Zug gemäß der jeweiligen Regeln auf einem bestimmten Brett erlaubt ist.
					\item \textbf{+ hasEnded(BoardState board): boolean}\\ Soll prüfen, ob das Spiel auf einem bestimmten Brett gemäß den jeweiligen Regeln beendet ist.
					\item \textbf{+ getResult(BoardState board): Result}\\ Soll das Ergebnis eines Spiels als \textbf{Result}zurückgeben. Ist das Spiel nicht beendet soll \textbf{null} zurückgegeben werden.\\
				\\
				\end{description}
			...
			Konkreter Regellieferer, welcher die genauen Schachregeln zur Verfügung stellt.\\
				...
				\begin{description}
					...
					\item \textbf{+ getLegalMoves(Position position, BoardState board): List<Move>}\\ Gibt eine Liste an erlaubten Zügen ausgehend von einer ausgewählten Position und einem Brett zurück. Dazu werden zunächst mit \textbf{getPossibleMoves(Position, BoardState)} alle möglichen Züge berechnet. Anschließen wird jeder Zug auf einer Kopie des Brettes simuliert, und mithilfe von \textbf{isChecked(boolean, BoardState)} überprüft, ob der selbe Spieler danach im Schach stünde (was den Zug ungültig machen würde).
					\item \textbf{+ isLegalMove(Move move, BoardState board): boolean}\\ Prüft, ob ein Zug gemäß den Schachregeln auf dem übergebenen Brett erlaubt ist.
					\item \textbf{+ isChecked(boolean white, BoardState board): boolean}\\ Überprüft ob ein Spieler auf dem gegebenen Brett im Schach steht. Ist der übergebene boolean true, wird Weiß überprüft, bei false Schwarz. Dazu wird geschaut, ob es eine gegnerische Figur gibt, welche durch \textbf{getPosssibleMoves(Position position, BoardState board)} einen Zug erhält, mit welchem der eigene König erreicht werden könnte.
					\item \textbf{+ hasEnded(BoardState board): boolean}\\ Prüft, ob das Schachspiel zu Ende ist. Dazu wird geprüft, ob der zu ziehende Spieler laut \textbf{isMate(BoardState board)} Matt gesetzt, laut \textbf{isStaleMate(BoardState board)} Patt gesetzt oder ob laut  \textbf{isDraw(BoardState board)} andersweitig ein Unentschieden erreicht wurde.
					...
					\item \textbf{- getPossibleMoves(Position position, BoardState board): List<Move>}\\ Gibt alle möglichen Züge einer Figur auf dem Brett zurück, ohne dabei zu berücksichtigen, ob der ziehende Spieler nach diesem Zug im Schach stehen würde. Dazu wird die jeweilige \textbf{getMovement()}-Methode der Figur an der angegebenen Position ausgeführt.				
					\item \textbf{- isMate(BoardState board): boolean}\\ Prüft, ob der zu ziehende Spieler Matt gesetzt wurde. Das ist der Fall, wenn der Spieler laut \textbf{isChecked(boolean, BoardState)} im Schach steht, und es für keine Figur einen nach \textbf{getLegalMoves(Position, BoardState)}erlaubten Zug gibt.
					\item \textbf{- isStaleMate(BoardState board): boolean}\\ Prüft, ob der zu ziehende Spieler Patt gesetzt wurde. Das ist der Fall, wenn der Spieler laut \textbf{isChecked(boolean, BoardState)} nicht im Schach steht, und es für keine Figur einen nach \textbf{getLegalMoves(Position, BoardState)}erlaubten Zug gibt.
					\item \textbf{- isDraw(BoardState board): boolean}\\ Prüft, ob ein anderweitiges Unentschieden erreicht wurde. Dies ist der Fall, wenn zu wenig Figuren auf dem Brett vorhanden sind um Matt zu setzen, oder wenn 50 Züge lang keine Figur geschlagen und kein Bauer gezogen wurde, also wenn der \textbf{movesWithoutAction}-Wert des \textbf{board}-Objekts 50 (oder größer) ist.
				\end{description}
		\end{itemize}
		\newpage
		...
		\begin{itemize}
			...
			\\
				...
				\rule{\textwidth}{1pt}
				\begin{description}
					\item \textbf{+ User(name: Strin)}\\ Erstellt einen neuen Benutzer mit dem übergebenen Namen. Setzt \textbf{elo} auf 1000, alle anderen Attribute auf 0. 
		...
		\begin{itemize}
			...
			\item \textbf{\Large{King}}
				...
				\begin{description}
					...
					\item \textbf{+ getMovement(Position p, BoardState b): List<Move>}\\ Gibt eine Liste der Züge zurück, die ein König auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann, inklusive Rochade. Dabei wird nicht berücksichtigt, ob sich der König nach dem Zug in Schach befinden würde.
				\end{description}
			...
			\item \textbf{\Large{Queen}}
				...
				\begin{description}	
					...
					\item \textbf{+ getMovement(Position p, BoardState b): List<Move>}\\ Gibt eine Liste der Züge zurück, die eine Dame auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug in Schach befinden würde.
				\end{description}
			...
			\item \textbf{\Large{Rook}}
				...
				\rule{\textwidth}{1pt}
				\begin{description}
					\item \textbf{- CHAR: String}\\ Die Repräsentation eines Turms, wird auf T gesetzt.
				...
				\begin{description}	
					...
					\item \textbf{+ getMovement(Position p, BoardState b)}\\ Gibt eine Liste der Züge zurück, die ein Turm auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug in Schach befinden würde.
				\end{description}
			...
			\item \textbf{\Large{Bishop}}
				...
				\rule{\textwidth}{1pt}
				\begin{description}			
					\item \textbf{- CHAR: String}\\ Die Repräsentation eines Läufers, wird auf L gesetzt.
				...
				\begin{description}				
					...
					\item \textbf{+ getMovement(Position p, BoardState b)}\\ Gibt eine Liste der Züge zurück, die ein Läufer auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug in Schach befinden würde.
				\end{description}
			...
			\item \textbf{\Large{Knight}}
				...
				\begin{description}			
					\item \textbf{- CHAR: String}\\ Die Repräsentation eines Springers, wird auf S gesetzt.
				...
				\begin{description}
					...
					\item \textbf{+ getMovement(Position p, BoardState b)}\\ Gibt eine Liste der Züge zurück, die ein Springer auf dem gegebenen Schachbrett von der gegebenen Position aus ausführen kann. Dabei wird nicht berücksichtigt, ob sich der König des ziehenden Spielers nach dem Zug in Schach befinden würde.
				\end{description}
			...
			\item \textbf{\Large{Pawn}}
				...
				\begin{description}				
					\item \textbf{- CHAR: String}\\ Die Repräsentation eines Bauerns, wird auf B gesetzt.
