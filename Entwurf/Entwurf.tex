...
\begin{document}
	...
	\section{Entwurfsentscheidungen}
		Das haben wir so gemacht und mit diesem Entwurfsmuster weil so und so und 
		das wurde geändert und das hat diesen Vorteil und jenen Nachteil xDDDDDDDDDDDDDDDDDDDDDDDDDDDD
		\subsection{Paketdiagramm}
		
		\subsection{\gls{GUI}}
		
		\subsection{Server}
		Der Server wurde nach dem \gls{REST} Prinzip entworfen. Alle Anfragen werden per \gls{URL} gesendet. Benachrichtigungen über neue Züge erfolgen über eine \gls{WS}-Verbindung.
	...
	\section{Klassendiagramme}
		...
		\textbf{\Large{Spiel}}
				\begin{description}
					\item \textbf{Game} stellt ein Spiel dar. Es besteht aus zwei Spielern \textbf{Player} und einem Spielfeld \textbf{BoardState}. Außerdem benutzt es eine Instanziierung des Interface \textbf{RuleProvider}, welches die Regeln für ein beliebiges Spiel auf einem Schachbrett liefert.
					\item In diesem Fall wird das Interface nur von der Klasse \textbf{ChessRuleProvider} implementiert, welche die Schachregeln liefert. Diese Klasse benutzt als Rückgabewert einer Methode die Klasse \textbf{Result}.
					\item Ein \textbf{Player} ist ein Spieler eines konkretes Spiels und besteht aus einem \textbf{User}, welcher einen Benutzer im Allgemeinen darstellt. Umgekehrt muss ein \textbf{User} aber nicht zu jedem Zeitpunkt einen \textbf{Player} haben.
					\item Das Spielfeld \textbf{BoardState} enthält 64  \textbf{Tiles} (Schachfelder) und einen \textbf{Move}, welcher hier den letzten ausgeführten Zug des Spiels darstellt. 
					\item Ein \textbf{Tile} kann ein \textbf{Piece} enthalten, muss es aber nicht.
					\item \textbf{Piece} ist eine abstrakte Klasse und stellt eine Figur dar. Sie wird von den sechs konkreten Figurenklassen beerbt.
					\item Ein \textbf{Move} besteht im Allgemeinen nur aus zwei \textbf{\textit{Positionen}}. Es gibt drei Spezialfälle eines Zuges, welche eine gesonderte Implementierung benötigen, da neben zwei \textbf{Positionen} noch andere Attribute enthalten.
					\item \textbf{Position} stellt eine Position auf einem Schachbrett dar und besteht aus zwei Zahlen, welche die Koordinaten darstellen. Diese müssen zwischen 0-7 liegen.
				\end{description}		
		\newpage
			  ...
			  \end{itemize}	
				...
				\end{description} 
	 			...
	 			\begin{description}
					...
					\item \textbf{+ getPossibleMoves(position: Position): List<Move>}\\ Gibt alle möglichen Züge als Liste zurück, welche eine Figur, die sich auf der übergebenen Position befindet, ausführen kann. Zum Berechnen dieser wird das \textbf{ruler} Objekt benutzt.
